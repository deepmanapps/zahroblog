...
describe('Comments Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./comments-detail.component').then(m => m.CommentsDetailComponent),
              resolve: { comments: () => of({ commentId: 123 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load comments on init', async () => {
      ...
      // THEN
      expect(instance.comments()).toEqual(expect.objectContaining({ commentId: 123 }));
    });
  });
