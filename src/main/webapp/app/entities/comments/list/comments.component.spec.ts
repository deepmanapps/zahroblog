...
import { sampleWithRequiredData } from '../comments.test-samples';
...
describe('Comments Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.comments?.[0]).toEqual(expect.objectContaining({ commentId: 123 }));
  });

  describe('trackCommentId', () => {
    it('Should forward to commentsService', () => {
      const entity = { commentId: 123 };
      jest.spyOn(service, 'getCommentsIdentifier');
      const commentId = comp.trackCommentId(0, entity);
      expect(service.getCommentsIdentifier).toHaveBeenCalledWith(entity);
      expect(commentId).toBe(entity.commentId);
    });
  });
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.query).toHaveBeenLastCalledWith(expect.objectContaining({ sort: ['commentId,desc'] }));
  });
